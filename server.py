...
class MudServer(object):
    ...
    # An inner class which is instantiated for each connected client to store info about them
    ...
    # Different states we can be in while reading data from client See _process_sent_data function
    ...
    def __init__(self):
        ...
        # set a special option on the socket which allows the port to be immediately without having to wait
        self._listen_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR,
                                       1)

        # set arbitary port 1234 as telnet port
    ...
    def update(self):
        #method for uptodate information
        ...
        # move the new events into the main events list 
    ...
    def send_message(self, to, message):
        # for always having newline for message
    ...
    def _check_for_new_connections(self):
        ...
        # 'accept' returns a new socket and address info which can be used to communicate with the new client
        ...
        # construct a new _Client object to hold info about the newly connected client. 
        ...
        # add 1 to 'nextid' so that the next client to connect will get an unique id
        self._nextid += 1
